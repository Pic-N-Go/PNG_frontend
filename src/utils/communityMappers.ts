...
export function mapPostDetail(dto: PostResponseDTO, exif: PostExifResponseDTO | undefined, ctx: PostMapContext = {}): PostDetail {
  ...
  // imageUrls와 길이를 맞춘다(인덱스로 꺼내 쓴다).
  const byImageId = new Map((exif?.images ?? []).map((e) => [e.imageId, e]));
  const exifList = (dto.images ?? []).map((img) => mapExif(byImageId.get(img.id), dto.shootingTime));
  return {
    ...mapPost(dto, ctx),
    exifList,
  };
