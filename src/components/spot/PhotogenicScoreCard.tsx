...
import { IconCalendar, IconChevronDown, IconClock, IconCloud, IconFlowerFilled, IconSun, IconWind } from '@tabler/icons-react-native';
...
import type { PhotogenicFactor, PhotogenicScoreData } from '@/types/spot';

const RING_SIZE = normalize(160);
const RING_RADIUS = 68;
const RING_CIRCUMFERENCE = 2 * Math.PI * RING_RADIUS;

const DEFAULT_DATE_LABEL = '2026.05.12 월';
const DEFAULT_TIME = new Date(2000, 0, 1, 6, 30);

// 목업엔 날짜/시간 선택 시 실제 예보를 다시 조회하는 로직이 없음 — API 연동 전 UI 셸만 우선 구현
// API팀 확인: 날짜는 당일 포함 3일(오늘/내일/모레), 시간은 24시간 임의 시각 조회 가능 (5분 단위로 선택)
// TODO: 예보 API 연동 시 선택값에 따라 PhotogenicScoreData를 다시 조회하도록 교체
const DATE_OPTIONS = ['2026.05.12 월', '2026.05.13 화', '2026.05.14 수'];
...
interface Props {
  spotName: string;
  data: PhotogenicScoreData;
}

export default function PhotogenicScoreCard({ spotName, data }: Props) {
  const progress = data.score / data.maxScore;
  const dash = RING_CIRCUMFERENCE * progress;
  const gap = RING_CIRCUMFERENCE - dash;

  const [selectedDate, setSelectedDate] = useState(DEFAULT_DATE_LABEL);
  ...
  return (
    <View style={{ paddingHorizontal: GRID_PADDING, paddingTop: normalize(24) }}>
      ...
      <View style={{ borderRadius: normalize(20), backgroundColor: '#F5F5F7', paddingTop: normalize(24), paddingBottom: normalize(20), paddingHorizontal: GRID_PADDING, alignItems: 'center' }}>
        <View
          style={{
            position: 'absolute',
            top: normalize(18),
        ...
        <View style={{ width: RING_SIZE, height: RING_SIZE, marginBottom: normalize(16) }}>
          ...
          >
            ...
            {/* 그라디언트 텍스트 대신 단색 근사 (마스킹 라이브러리 미설치) */}
        ...
        >
          ...
          <Text allowFontScaling={false} style={{ fontFamily: 'Pretendard-Regular', fontSize: normalizeFontSize(13), color: 'rgba(0,0,0,0.6)', letterSpacing: -0.15 }}>
            골든아워 <Text style={{ fontFamily: 'Pretendard-SemiBold', color: '#C77700' }}>{data.goldenHourMinutesLeft}분 후</Text> 시작 · {data.goldenHourTime}
          </Text>
        </View>
