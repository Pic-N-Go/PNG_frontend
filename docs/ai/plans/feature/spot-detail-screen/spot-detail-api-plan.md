...
- 변경 내용:
  ...
  - `ReviewTimeSlot = 'SUNRISE'|'DAYTIME'|'SUNSET'|'NIGHT'`(DTO 필드명은 `timePeriod`, `nullable`), `ReviewSortApi = 'LATEST'|'RATING_HIGH'|'RATING_LOW'`, `ChecklistItemDTO { id: number | null; content: string }`
