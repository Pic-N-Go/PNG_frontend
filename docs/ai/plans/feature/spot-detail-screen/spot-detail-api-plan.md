...
- 변경 내용:
  ...
  - `TimePeriodApi = 'SUNRISE'|'DAYTIME'|'SUNSET'|'NIGHT'`(DTO 필드명은 `timePeriod`, `nullable`), `ReviewSortApi = 'LATEST'|'RATING_HIGH'|'RATING_LOW'`, `ChecklistItemDTO { id: number | null; content: string }`
...
- 변경 내용:
  ...
  - `mapReviewList(dto) → { summary: ReviewSummaryData; reviews: Review[] }` (분포 카운트→percent, nickname→이니셜/색, timePeriod→라벨, **timePeriod null→배지 없음**, photos `{photoId,url}`→`url` 추출)
  - `SORT_TO_API`, `TIME_PERIOD_LABEL` 매핑 상수
- 완료 조건: 순수 함수, 부작용 없음
