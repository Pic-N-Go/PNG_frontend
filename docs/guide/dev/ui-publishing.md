...
- 콘테스트 세그먼트 하위 언더라인 서브탭 3개 — `switchSubtab('active'|'mine'|'past', el)`
  ...
  - **진행중은 단계에 따라 화면이 셋으로 갈립니다** — 7일 주기를 출품 4일 → 투표 2일 → 결과 1일로 나눕니다.
    목업은 우상단 `.phase-switch`로 전환해 확인하고, 실제로는 서버가 내려주는 단계 하나만 렌더합니다
    (`switchPhase('submit'|'vote'|'result', el)`)
    - **출품 기간**: 히어로 280px + 핑크 `출품 기간` 배지. **순위도 투표 버튼도 없습니다** — 겨루는 단계가 아니라 모으는 단계입니다.
      "새로 올라온 출품작" 최신순 그리드(카드 메타는 올라온 시각) + "투표는 N일부터" 안내 배너
    - **투표 기간**: 히어로 200px(순위 변동 펼치면 160px). 정렬은 **랜덤(기본) · 최신순 2개** —
      득표순을 빼고 카드에서도 순위·득표수를 지웁니다. 메타는 `스팟 · 출품 시각`.
      CTA는 `내가 투표한 작품 >` + `모두 보기` → `contest-all-entries.html`
    - **결과 발표**: 히어로 180px + 내 결과 요약(순위·득표수·공유) → 1위 큰 카드(334:200) → 2·3위 그리드.
      **순위 배지와 득표수는 이 단계에서만 다시 씁니다**
  - 순위 변동 섹션(`.rank-panel`)은 투표 기간에만, 히어로 바로 아래 최상단에 둡니다
    - 정렬에서 득표순을 빼고 카드에서 득표수를 지운 대신, 하루 3번(09:00·12:00·18:00) 집계된 **1~3위만** 여기서 공개합니다
    - 접힘 72px — 썸네일 3개 10px 겹침 + `12:00 기준 · @닉네임 1위` + chevron.
      펼침 — `12:00 업데이트` 배지 + 3시점 × 3위 그래프 + 시각 세그먼트 + 범례 3행
    - 그래프 선은 1위만 accent, 나머지는 `#b8b8be` / `#d2d2d8`. **마지막 집계 이후의 실시간 득표수는 노출 금지**
    - 그래프 점은 `left`를 비율(`15.686% · 52.614% · 89.542%` = viewBox 306 기준 x/306)로 둡니다.
      고정 px로 두면 360dp에서 우측 점이 밖으로 나갑니다
    - 집계 직후 1시간은 자동 펼침
  - **투표 취소는 투표 기간 안에서 자유입니다** — 완료 버튼 재탭으로 표 1개 복구, 기간 종료 시 확정.
    (초기 시안의 "되돌리기 없음"은 폐기됨)
  - 투표 피드백은 목록 화면과 동일 — 색 반전 + 토스트 + 햅틱
  - **내가 투표한 작품 시트**(`#myvotes-sheet`)는 하단 CTA `내가 투표한 작품 >`과 정렬 바의 **남은 표 도트**
    두 곳에서 열립니다. 도트도 진입점이라 `<button>`으로 감쌉니다
...
- **투표 기간에만 열리는 화면입니다.** 출품 기간·결과 발표에는 진입 경로가 없습니다
- 정렬은 칩(pill)이 아니라 배경 없는 텍스트 2개(랜덤 · 최신순), 구분점 3px. 기본은 **랜덤**
  - 득표순은 없습니다 — 순위는 진행중 탭의 순위 변동 섹션에서만 공개하고, 이 화면은 훑어보며 투표하는 곳입니다
  - 득표순이 있으면 상위권만 표를 더 받아 새 출품작이 영영 노출되지 않는 문제도 함께 생깁니다
...
- 투표 버튼 28px 원형 3상태 — 기본 accent+흰 thumbs-up / 완료 `rgba(227,27,89,.10)`+accent check / 소진 `#e6e6ea`+`#b8b8be`
  - **완료 버튼 재탭으로 취소** — 표 1개가 복구되고, 기간 종료 시 확정됩니다
...
- 투표 피드백은 세 겹입니다 — 버튼 색 반전 + 토스트(`@rimi 님에게 투표했어요 · 2/3`) + 햅틱
  - 표가 하루 3개뿐이라 소비된 감각이 즉시 보여야 해서 남은 표를 문구에 함께 넣습니다
...
- **내가 투표한 작품 시트** (시안 6a·6b) — 남은 표 도트 탭으로 열림. 진행중 탭과 같은 시트입니다
  - 스크림 `rgba(0,0,0,.34)` · 시트 radius `24 24 0 0` · 핸들 36×4 `#e6e6ea`
  - 제목 `오늘 투표한 작품 n`(개수만 `#c7c7cc`), 행은 썸네일 44px(radius 11) + 닉네임 + `오늘 14:20 · 스팟` + 취소 pill(32px)
  - 푸터: 남은 표 도트 + `내일 0시에 3표로 초기화`. 표가 0개면 푸터를 감추고 빈 상태를 보여줍니다
  - **취소는 다이얼로그 없이 즉시 반영**하고 토스트로만 알립니다. 그리드·도트·시트가 함께 갱신됩니다
  - 기간 종료 후에는 취소 버튼을 없애고 제목을 `내가 투표한 작품`으로 바꿉니다
- **지난 콘테스트 변형** (시안 1b) — 결과가 확정된 뒤의 같은 화면입니다
  - 정렬 `득표순(기본) · 최신순`, 사진 좌상단 **순위 배지 부활**, 메타는 `n표 · 스팟`
  - **투표 버튼과 남은 표 인디케이터가 없고**, 그 자리에 `8월 12일 마감`이 들어갑니다
