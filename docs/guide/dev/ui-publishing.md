...
src/components/ui/
  ...
  community/
    ...
    contest-result.html       # 콘테스트 결과 (10b 수상작 상세·10d 축하·10f 순위권 밖 / 풀스크린. 10a 요약은 미채택)
...
| `--font-title` | `20px` | `FONT_TITLE` | 페이지 내 섹션 타이틀·시트 제목 |
| `--font-xl` | `22px` | `FONT_XL` | 더 큰 표제 (콘테스트 표시 텍스트 등) |
| `--font-2xl` | `28px` | `FONT_2XL` | 강조 대제목 |

> **폰트 크기는 이 9개 토큰(`10 · 11 · 13 · 14 · 15 · 17 · 20 · 22 · 28px`) 안에서만 고릅니다.**
> 섹션·시트 제목은 `20px`(`--font-title` / `FONT_TITLE`), 더 큰 표제는 `22px`.
> 그 사이값(`9 · 12 · 16 · 18px` 등)은 토큰에 없으므로 새로 만들어 쓰지 않습니다.
...
- 콘테스트 세그먼트 하위 언더라인 서브탭 3개 — `switchSubtab('active'|'mine'|'past', el)`
  - 콘테스트에서만 헤더 고정을 풀고 **서브탭 행만 sticky** (`.phone-scroll.is-contest`) —
    ...
    같은 이유로 콘테스트 탭에서는 헤더 검색 아이콘과 `+` 버튼을 함께 숨깁니다 — 검색 대상이 게시글·갤러리이고,
    `+`도 게시글 작성으로 가서 이 탭과 무관합니다(콘테스트 출품은 탭 안에 자체 CTA가 있음)
...
- 콘테스트 결과 — 다음 달 1일 발표 후 **한 달 내내** 유지됩니다 (시안 10b·10d·10f)
...
- 본문 공통: 내 결과 카드 → 1위 큰 카드(334:220) → 2·3위 2열 그리드.
  출품하지 않은 달이면 내 결과 카드를 감춥니다
- 10d 수상(1~3위): 최상단이 축하 카드(`rgba(227,27,89,.06)` · radius 20)로 바뀝니다. 순위 리스트에서 내 행의 숫자만 accent.
  **4위 이하는 10f를 씁니다** — 축하 연출을 등수와 무관하게 쓰면 의미가 없어집니다.
  컨페티나 별도 모달은 쓰지 않습니다
- 10f 순위권 밖(4위 이하): 공유 버튼을 넣지 않습니다 — 42위를 공유하라고 권하는 것은 무례합니다.
