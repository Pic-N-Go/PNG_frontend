...
**GPS 제거 방침은 2026-07-29에 철회됐습니다.** 사진 정보 화면에서 촬영 주소와 지도를 표시하기로 결정해, 애플리케이션은 전달받은 EXIF를 임의로 제거하지 않습니다.
 다만 운영체제나 사진 선택기가 개인정보 보호를 위해 GPS를 가릴 수 있으므로 위치 필드는 항상 nullable입니다.

- 업로드 시 전달된 EXIF 보존 — 촬영 정보(카메라, 렌즈, 날짜, GPS)를 저장합니다
- 촬영 일시, 카메라 모델, 조리개/셔터스피드/ISO도 보관
- 유효한 위도·경도는 카카오 좌표 변환 API로 역지오코딩해 `address`에 저장
...
커뮤니티와 리뷰 사진의 EXIF 조회 엔드포인트는 **인증 없이 조회**되므로 촬영 좌표와 주소가
공개될 수 있습니다. 사용자가 스팟이 아닌 곳(집 등)에서 찍은 사진을 첨부하면 그 위치가
노출됩니다.
