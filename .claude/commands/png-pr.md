...
5. 사용자 응답에 따라 분기합니다.
   - **승인**: 원격 추적 브랜치가 없으면 `git push -u origin <current-branch>`, 있으면 로컬이 앞서 있는지 확인 후 필요 시 `git push`. 이후 `gh pr create --title "<제목>" --body "<승인된 본문>" --base main --head <current-branch>` 로 PR을 생성하고 결과 URL을 보고합니다.
...
6. 승인 후 `gh pr create`가 성공했고, 4번 이전 단계에서 임시 저장된 초안 파일이 있다면 `rm -- docs/ai/pr-drafts/<해당 파일>` 로 정리합니다 (PR이 최신 기록이므로 중복 파일을 남기지 않음).
