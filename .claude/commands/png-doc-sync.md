...
4. 고정 점검 문서를 Read합니다.
   - 필수: `docs/ai/README.md`, `docs/ai/07-harness-self-audit-checklist.md`, `docs/ai-prompt-guide.md`, `CLAUDE.md`, `.github/CONVENTIONS.md`
   - 변경 파일에 feature 관련 spec/plan이 있으면 해당 `docs/ai/specs/`, `docs/ai/plans/` 파일도 Read합니다.
...
7. 4-section 리포트를 출력하고 수정 제안과 함께 사용자 승인을 요청합니다.
   ...
   - `$ARGUMENTS`에 `--save`가 포함된 경우: 현재 브랜치명(`git branch --show-current`)을 확인하고, Write 전에 `mkdir -p docs/ai/doc-sync-reports`를 실행해 디렉토리를 생성한 후 `docs/ai/doc-sync-reports/<branch>-sync.md`에 리포트를 저장합니다.
